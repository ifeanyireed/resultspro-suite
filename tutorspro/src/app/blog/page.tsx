...
export default function BlogListPage() {
  ...
  const fetchPosts = async () => {
    try {
      ...
      // Map API response to match interface if necessary
      const mappedPosts = (res.data || []).map((p: any) => ({
        ...p,
        featuredImage: p.featured_image || p.featuredImage, // Support both snake and camel
        publishedAt: p.published_at || p.publishedAt,
        author: p.author ? { name: p.author.full_name || p.author.name } : { name: "Admin" },
        category: p.category ? { name: p.category.name } : { name: "General" }
      }));
