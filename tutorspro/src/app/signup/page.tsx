...
export default function SignupPage() {
  const [studentCount, setStudentCount] = useState("10,000+");

  useEffect(() => {
    const fetchStats = async () => {
      try {
        const res = await api.get('/public/home');
        const stats = res.data?.hero?.stats;
        const studentStat = stats?.find((s: any) => s.label === 'Active Students');
        if (studentStat) setStudentCount(studentStat.value);
      } catch (err) {
    ...
    <main className="min-h-screen relative flex flex-col items-center justify-center px-4 py-20 overflow-hidden text-black dark:text-white">
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="text-center mb-10 flex flex-col items-center">
          ...
          <p className="text-gray-500">Join {studentCount} students and start winning.</p>
        </div>
