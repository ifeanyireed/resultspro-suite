...
function LoginForm() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const res = await api.post(`${ process.env.NEXT_PUBLIC_USERS_API || 'https://resultsproserviceusers.vercel.app' }/api/v1/auth/login`, formData);
  ...
  const loginWithGoogle = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      setIsLoading(true);
      try {
        const res = await api.post(`${ process.env.NEXT_PUBLIC_USERS_API || 'https://resultsproserviceusers.vercel.app' }/api/v1/auth/google`, { idToken: tokenResponse.access_token });
        const user = res.data.user;
        setAuth(user, res.data.access_token || res.data.token);
  ...
  const handleMicrosoftLogin = async () => {
    ...
    try {
      ...
      const res = await api.post(`${ process.env.NEXT_PUBLIC_USERS_API || 'https://resultsproserviceusers.vercel.app' }/api/v1/auth/microsoft`, { accessToken: loginResponse.accessToken });
      const user = res.data.user;
      setAuth(user, res.data.access_token || res.data.token);
