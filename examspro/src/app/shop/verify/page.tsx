...
function VerifyContent() {
  ...
  return (
    <div className="max-w-xl w-full mx-auto p-8 rounded-[40px] bg-white border border-slate-200 shadow-xl text-center">
      {status === 'loading' && (
        <div className="py-12 space-y-6">
          <div className="relative">
             <div className="w-20 h-20 border-4 border-slate-100 border-t-blue rounded-full animate-spin mx-auto" />
             <ShieldCheck className="w-10 h-10 text-blue absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" />
          </div>
          <div>
            <h2 className="text-2xl font-display font-black text-slate-900 mb-2">Verifying Payment</h2>
      ...
      {status === 'success' && (
        <div className="py-12 space-y-8 animate-in fade-in zoom-in duration-500">
          ...
          <div>
            <h2 className="text-3xl font-display font-black text-slate-900 mb-2 uppercase">Payment Successful!</h2>
          ...
          <div className="p-6 rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-between">
             <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                   ...
                   <div className="text-lg font-bold text-slate-900">{details.coins?.toLocaleString()} Coins</div>
                </div>
             </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Button 
              ...
              className="py-6 rounded-2xl bg-slate-50 text-slate-700 border border-slate-200 hover:bg-slate-100 font-bold"
            >
              Dashboard
            </Button>
            <Button 
              onClick={() => router.push('/shop')}
              className="py-6 rounded-2xl bg-[#146ef5] text-white hover:bg-blue-600 shadow-lg shadow-blue-500/20 font-bold flex items-center justify-center gap-2"
            >
              Shop More
      ...
      {status === 'error' && (
        <div className="py-12 space-y-8 animate-in fade-in slide-in-from-top-4 duration-500">
          ...
          <div>
            <h2 className="text-3xl font-display font-black text-slate-900 mb-2 uppercase">Verification Failed</h2>
            <p className="text-red-500 font-bold">{details.error}</p>
            <p className="text-gray-500 text-sm mt-4 italic">
              If you were already debited, please contact support at <span className="text-slate-900 font-bold">hello@resultspro.ng</span> with your reference: <span className="text-slate-900 font-mono bg-slate-100 px-2 py-1 rounded">{reference}</span>
            </p>
          </div>
          ...
          <div className="pt-4">
            <Button 
               onClick={() => router.push('/shop')}
               className="w-full py-6 rounded-2xl bg-slate-50 text-slate-700 border border-slate-200 hover:bg-slate-100 font-bold"
            >
              Return to Shop
            </Button>
          </div>
