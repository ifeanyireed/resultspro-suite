  ...
  return (
    <main className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-[#146ef5]/10 rounded-full blur-[120px] transition-all duration-1000 ${(status === 'found' || status === 'ready') ? 'bg-green-500/20 scale-125' : ''}`} />
          ...
          <div className="text-center flex flex-col items-center gap-2">
            ...
            {battleIdParam && (
              <div className="mt-2 p-2 px-4 rounded-xl bg-slate-100 border border-gray-200  flex flex-col items-center animate-in fade-in slide-in-from-top-2 duration-700">
                ...
                >
                  <span className="text-2xl font-display font-black tracking-[0.2em] text-gray-900 group-hover:text-green-500">
                    {roomDetails?.roomCode || battleIdParam.split('-')[0].toUpperCase()}
                  </span>
                  {copied ? (
                    <Check className="w-3 h-3 text-green-500" />
                  ) : (
                    <Copy className="w-3 h-3 text-gray-500 group-hover:text-green-500" />
                  )}
                </button>
                ...
                <div key={p.userId || i} className="flex flex-col items-center gap-5 animate-in zoom-in duration-500">
                  <div className="relative group">
                    ...
                    <div className={`absolute -bottom-3 left-1/2 -translate-x-1/2 px-4 py-1.5 rounded-2xl font-black text-[10px] border-2 border-navy shadow-xl z-20 whitespace-nowrap transition-all ${
                      isMe ? 'bg-[#146ef5] text-gray-900 scale-110' : 'bg-gray-800 text-gray-500'
                    }`}>
                      {isMe ? 'YOU' : 'CONTENDER'}
                    </div>
            ...
            <div className="p-6 rounded-[32px] bg-slate-100 border border-gray-200  backdrop-blur-xl backdrop-saturate-[1.2] shadow-[0_8px_32px_0_rgba(0,0,0,0.36)] shadow-[inset_0_1px_0_rgba(255,255,255,0.1)] text-center min-w-[320px]">
              ...
              {isHost ? (
                <Button
                  onClick={startBattle}
                  disabled={!canStart}
                  className={`w-full py-8 rounded-2xl font-black text-xl shadow-2xl transition-all ${canStart ? 'bg-green-500 text-white hover:scale-105 shadow-green/20' : 'bg-slate-100 text-gray-400 cursor-not-allowed'}`}
                >
              ...
              <button 
                onClick={handleLeave}
                className="mt-4 text-[10px] font-bold text-gray-400 hover:text-red-500 uppercase tracking-[0.2em] transition-colors"
              >
