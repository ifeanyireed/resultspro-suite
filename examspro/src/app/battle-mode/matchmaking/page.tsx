  ...
  return (
    <main className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-secondary/10 rounded-full blur-[120px] transition-all duration-1000 ${(status === 'found' || status === 'ready') ? 'bg-secondary/20 scale-125' : ''}`} />
      ...
      <div className="relative z-10 w-full max-w-4xl">
        ...
        <div className="flex justify-between items-center mb-16 opacity-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-sm font-bold text-gray-900">{stake} Coins</div>
            </div>
          </div>
          ...
          <div className="text-center flex flex-col items-center gap-2">
            <div>
              ...
              <div className="text-sm font-bold text-gray-900 uppercase tracking-tighter">{subjectName}</div>
            </div>
            ...
            {battleIdParam && (
              <div className="mt-2 p-2 px-4 rounded-xl bg-slate-100 border border-gray-200  flex flex-col items-center animate-in fade-in slide-in-from-top-2 duration-700">
                <div className="text-[8px] font-bold text-secondary uppercase tracking-[0.2em] mb-1">Room Code</div>
                <button 
                  onClick={copyToClipboard}
                  className="flex items-center gap-2 group hover:text-secondary transition-colors"
                >
                  <span className="text-2xl font-display font-black tracking-[0.2em] text-gray-900 group-hover:text-secondary">
                    {roomDetails?.roomCode || battleIdParam.split('-')[0].toUpperCase()}
                  </span>
                  {copied ? (
                    <Check className="w-3 h-3 text-secondary" />
                  ) : (
                    <Copy className="w-3 h-3 text-gray-500 group-hover:text-secondary" />
                  )}
                </button>
              ...
              return (
                <div key={p.userId || i} className="flex flex-col items-center gap-5 animate-in zoom-in duration-500">
                  <div className="relative group">
                    ...
                    {isMe && (
                      <div className="absolute -inset-4 bg-secondary/20 rounded-[48px] blur-2xl animate-pulse" />
                    )}
                    ...
                    <div className={`w-28 h-28 md:w-36 md:h-36 rounded-[40px] p-1.5 flex items-center justify-center transition-all duration-700 border-4 relative z-10 overflow-hidden ${
                      isMe ? 'bg-secondary/20 border-secondary rotate-3 scale-105 shadow-[0_0_40px_rgba(21,101,192,0.4)]' : 
                    ...
                    <div className={`absolute -bottom-3 left-1/2 -translate-x-1/2 px-4 py-1.5 rounded-2xl font-black text-[10px] border-2 border-navy shadow-xl z-20 whitespace-nowrap transition-all ${
                      isMe ? 'bg-secondary text-secondary-foreground scale-110' : 'bg-gray-800 text-gray-500'
                    }`}>
                      {isMe ? 'YOU' : 'CONTENDER'}
                    </div>
                  ...
                  <div className="text-center space-y-1 relative z-10">
                    <h2 className={`text-xl font-display font-black tracking-tight transition-colors ${isMe ? 'text-gray-900' : 'text-gray-500 group-hover:text-gray-900'}`}>
                      {p.user?.name || p.user?.email?.split('@')[0] || "Joining..."}
                    </h2>
                    <div className="flex items-center justify-center gap-2">
                      <div className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest border ${isMe ? 'bg-secondary/10 border-secondary/30 text-secondary' : 'bg-slate-100 border-gray-200  text-gray-400'}`}>
            ...
            {Array.from({ length: Math.max(0, maxPlayers - displayParticipants.length) }).map((_, i) => (
              <div key={`empty-${i}`} className="flex flex-col items-center gap-5 opacity-20 group">
                <div className="relative">
                  <div className="w-28 h-28 md:w-36 md:h-36 rounded-[40px] border-4 border-dashed border-gray-200  flex items-center justify-center bg-white transition-all group-hover:border-gray-300">
                    <Users className="w-12 h-12 text-gray-900/5" />
                  </div>
                </div>
                <div className="text-center">
                  <div className="text-sm font-black text-gray-400 uppercase tracking-widest animate-pulse">Waiting...</div>
                </div>
              </div>
            ))}
          </div>
        ...
        {battleIdParam && status === 'idle' && (
          <div className="flex flex-col items-center gap-6 animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-500 mt-12">
            <div className="p-6 rounded-[32px] bg-slate-100 border border-gray-200  backdrop-blur-xl backdrop-saturate-[1.2] shadow-[0_8px_32px_0_rgba(0,0,0,0.36)] shadow-[inset_0_1px_0_rgba(255,255,255,0.1)] text-center min-w-[320px]">
              <div className="text-xl font-display font-bold text-gray-900 mb-2">{participantCount} Players Joined</div>
              ...
              {isHost ? (
                <Button
                  onClick={startBattle}
                  disabled={!canStart}
                  className={`w-full py-8 rounded-2xl font-black text-xl shadow-2xl transition-all ${canStart ? 'bg-secondary text-secondary-foreground hover:scale-105 shadow-secondary/20' : 'bg-slate-100 text-gray-400 cursor-not-allowed'}`}
                >
                ...
                <div className="w-full py-6 rounded-2xl bg-slate-100 border border-gray-200  flex flex-col items-center justify-center gap-2">
                   ...
                   <div className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Match will begin shortly</div>
                </div>
              )}

              <button 
                onClick={handleLeave}
                className="mt-4 text-[10px] font-bold text-gray-400 hover:text-primary uppercase tracking-[0.2em] transition-colors"
              >
          ...
          <div className="mt-24 text-center h-20 flex flex-col items-center justify-center">
            {status === 'searching' && (
              <p className="text-gray-500 text-sm max-w-xs leading-relaxed animate-in fade-in slide-in-from-bottom-4 duration-1000">
                Matching you with an available student in the <span className="text-gray-900">{subjectName}</span> queue...
              </p>
            )}
          </div>
        )}
