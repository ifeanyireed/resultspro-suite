  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
        ...
        <Link href="/login">
          <button className="inline-flex justify-center items-center bg-[#146ef5] hover:bg-[#105bd1] text-white rounded-full px-12 py-6 text-lg font-bold shadow-sm shadow-[#146ef5]/20 transition-all">
            Sign In to Continue
          </button>
        </Link>
      </div>
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-2">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 tracking-tight">
            Welcome back, {data.user?.name || user?.name || 'Student'}!
          </h1>
          <p className="text-sm text-gray-500 mt-1">
            Targeting {data.user?.target || 'Your Exam'} • {data.user?.daysToGo || 0} days to go
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-2">
        ...
        <WhiteMetricCard 
          title="Study Streak"
          value={`${data.user?.streak || user?.streakCurrent || 0} Days`}
          trend="🔥 Keep it up!"
          icon={IconFlame}
        />
        <WhiteMetricCard 
            ...
            <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
              {Array.isArray(data.subjects) && data.subjects.length > 0 ? data.subjects.map((sub) => (
                ...
                <div className="w-full py-8 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                  ...
                  <Link href="/practice">
                    <button className="inline-flex justify-center items-center px-4 py-2 text-sm font-semibold rounded-full text-[#146ef5] border border-[#146ef5] bg-white hover:bg-blue-50 transition-colors">Explore Subjects</button>
                  </Link>
                </div>
              )}
            </div>
          ...
          >
            {Array.isArray(battleHistory) && battleHistory.length > 0 ? (
              ...
              <div className="py-8 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                ...
                <Link href="/battle-mode">
                  <button className="inline-flex justify-center items-center px-4 py-2 text-sm font-semibold rounded-full text-[#146ef5] border border-[#146ef5] bg-white hover:bg-blue-50 transition-colors">Go to Battle Arena</button>
                </Link>
              </div>
            )}
          </WidgetCard>
          ...
          <WidgetCard title="Leaderboard" subtitle="Global Top 5">
            <div className="space-y-3">
              {Array.isArray(data.leaderboard) && data.leaderboard.map((u) => (
                <div key={u.rank} className="grid grid-cols-[30px_1fr_60px] gap-2 items-center p-3 rounded-xl hover:bg-gray-50 transition-colors">
            ...
            <Link href="/leaderboard">
              <button className="inline-flex justify-center items-center px-4 py-2 text-sm font-semibold rounded-full w-full mt-4 text-gray-500 hover:text-gray-900 hover:bg-gray-100 transition-colors">
                View Full Rankings
              </button>
            </Link>
          </WidgetCard>
