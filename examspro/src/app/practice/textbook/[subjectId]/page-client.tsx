...
import { IconArrowLeft as ArrowLeft, IconLoader2 as Loader2, IconAlertCircle as AlertCircle, IconBook as Book, IconPlus as Plus, IconMinus as Minus, IconMaximize as Maximize, IconTypography as Type, IconFileText as FileText } from '@tabler/icons-react';
...
export default function TextbookViewerPage() {
  ...
  if (error || !subject?.textbookUrl) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-8 text-center">
        ...
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Textbook Not Found</h1>
        <p className="text-gray-500 mb-8 max-w-md">The requested textbook is either missing or has not been assigned to this subject yet.</p>
        <button 
          onClick={() => router.back()}
          className="px-8 py-4 rounded-2xl bg-white border border-gray-200 shadow-sm text-gray-900 font-bold hover:bg-gray-100 transition-all flex items-center gap-2"
        >
  ...
  return (
    <main className="h-screen bg-slate-50 flex flex-col overflow-hidden">
      {/* Top Header Bar */}
      <header className="h-20 bg-white border-b border-gray-200 shadow-sm backdrop-blur-xl flex items-center justify-between px-6 md:px-10 shrink-0 z-50">
        <div className="flex items-center gap-6">
          <button 
            onClick={() => router.back()}
            className="p-3 rounded-xl bg-white border border-gray-200 shadow-sm text-gray-500 hover:text-gray-900 hover:bg-gray-100 transition-all"
            title="Go Back"
          >
          ...
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              <Book className="w-3.5 h-3.5 text-[#146ef5]" />
              <span className="text-[10px] font-black text-[#146ef5] uppercase tracking-widest">{subject.name}</span>
            </div>
            <h1 className="text-sm md:text-lg font-bold text-gray-900 truncate max-w-[200px] md:max-w-md leading-none">
          ...
          <>
            ...
            <div className="fixed right-8 bottom-12 z-50 flex flex-col gap-2">
              <button 
                ...
                className="p-3 rounded-xl bg-gray-100 backdrop-blur-xl backdrop-saturate-[1.2] shadow-[0_8px_32px_0_rgba(0,0,0,0.36)] shadow-[inset_0_1px_0_rgba(255,255,255,0.1)] border border-gray-300 text-gray-900 hover:bg-white/20 transition-all shadow-xl"
                title="Zoom In"
              >
