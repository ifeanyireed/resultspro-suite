...
export default function SyllabusClient({ syllabus, examId }: { syllabus: any, examId: string }) {
  ...
  if (!syllabus || !syllabus.exam) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
         ...
         <h1 className="text-3xl font-display font-black text-gray-900 mb-2">Syllabus Not Found</h1>
         ...
         <button onClick={() => router.back()} className="px-8 py-3 rounded-xl bg-white border border-gray-200 shadow-sm text-gray-900 font-bold hover:bg-gray-100 transition-all">
           Go Back
         </button>
      </div>
    );
  ...
  return (
    <main className="flex-1 max-w-5xl mx-auto w-full px-6 py-12">
      <Link href={`/practice/${examId}`} className="inline-flex items-center gap-2 text-gray-500 hover:text-gray-900 mb-8 transition-colors font-bold text-sm">
      ...
      <header className="mb-12">
         <div className="flex items-center gap-4 mb-4">
            ...
            <div>
               <h1 className="text-4xl font-display font-black text-gray-900">{exam.name}</h1>
      ...
      <div className="space-y-6">
         {exam.subjects?.map((subject: any) => {
           ...
           return (
             <section key={subject.id} className="rounded-[40px] bg-white border border-gray-200 shadow-sm overflow-hidden transition-all">
                <button 
                ...
                >
                   <div className="flex items-center gap-3 md:gap-4">
                      ...
                      <div>
                        <h2 className="text-xl md:text-2xl font-bold text-gray-900">{subject.name}</h2>
                        <div className="text-[10px] font-black text-gray-500 uppercase tracking-widest mt-1">
                           {subject.topics?.length || 0} Topics
                        </div>
                      </div>
                   </div>
                   <div className="p-2 rounded-full bg-white border border-gray-200 shadow-sm text-gray-500">
                ...
                {isExpanded && (
                  <div className="px-4 md:px-8 pb-8 pt-2 space-y-3">
                    {subject.topics?.map((topic: any, idx: number) => (
                      <div key={topic.id} className="p-4 md:p-6 rounded-2xl bg-white border border-gray-200 hover:border-[#146ef5]/30 hover:bg-slate-50 transition-all group">
                         <div className="flex items-start gap-3 md:gap-4">
                            <span className="text-xs font-mono text-gray-600 font-bold mt-1">{String(idx + 1).padStart(2, '0')}</span>
                            <div className="flex-1">
                               <h3 className="text-base md:text-lg font-bold text-gray-600 group-hover:text-gray-900 transition-colors mb-2">{topic.name}</h3>
