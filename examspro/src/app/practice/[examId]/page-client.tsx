...
`}</style>
            ...
            >
              {filteredSubjects.map((sub, i) => (
                ...
                <Link
                  href={sub.isLocked ? "#" : `/practice/${examId}/${sub.id}`}
                  onClick={(e) => {
                    if (sub.isLocked) {
                      e.preventDefault();
                  ...
                  className={`group p-6 rounded-[1.5rem] shadow-sm hover:-translate-y-1 transition-transform overflow-hidden relative flex flex-col h-full ${sub.isLocked ? 'opacity-60 grayscale cursor-not-allowed' : ''}`}
                >
                  ...
                  <div className="flex justify-between items-start mb-6">
                    <div className="icon-box w-12 h-12 rounded-2xl flex items-center justify-center bg-white/20 backdrop-blur-sm text-white group-hover:scale-110 group-hover:bg-white transition-all">
                      {sub.isLocked ? <Lock className="w-6 h-6" /> : <BookOpen className="w-6 h-6" />}
                    </div>
