    ...
    <main className="min-h-screen bg-white pb-24">
      ...
      <div className="max-w-2xl mx-auto px-4 pt-12">
        <div className="flex justify-between items-end mb-10">
          <div>
            <h1 className="text-3xl font-display font-bold text-navy mb-2">Notifications</h1>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-slate-50 rounded-[40px] border border-slate-200">
            <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center border border-slate-200 shadow-sm mx-auto mb-6">
              <Bell className="w-8 h-8 text-gray-500" />
            </div>
            <h3 className="text-navy font-bold text-lg mb-2">All caught up!</h3>
              ...
              return (
                <div 
                  ...
                  className={`
                    ...
                    ${!n.isRead ? 'bg-white border-slate-200 shadow-lg' : 'bg-slate-50 border-slate-200 opacity-70'}
                  `}
                >
                  {!n.isRead && (
                    <div className="absolute top-6 right-8 w-2 h-2 rounded-full bg-red-600 shadow-[0_0_10px_rgba(220,38,38,0.5)]" />
                  )}
                  ...
                  <div className="flex gap-6">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-start mb-1">
                        <h3 className="font-bold text-navy group-hover:text-red-600 transition-colors">{n.title}</h3>
                      ...
                      {n.type === 'battle' && !n.isRead && (
                        <div className="flex gap-2 mt-4">
                          <Button className="bg-red-600 text-white hover:bg-red-700 rounded-xl px-6 h-10 text-xs font-bold">Accept Battle</Button>
                          <Button variant="outline" className="border-slate-200 text-gray-500 hover:bg-slate-100 rounded-xl px-6 h-10 text-xs font-bold">Decline</Button>
                        </div>
                      )}
                    </div>
                  </div>
        ...
        {notifications.length > 0 && (
          <div className="mt-12 text-center">
            <p className="text-xs text-gray-400 font-bold uppercase tracking-[0.2em]">End of notifications</p>
          </div>
        )}
      </div>
    </main>
