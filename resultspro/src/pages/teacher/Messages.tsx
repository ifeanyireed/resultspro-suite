...
import { IconSearch as Search, IconSend as Send, IconUser as User, IconMessageCircle as MessageCircle, IconChevronRight as ChevronRight, IconClock as Clock, IconArrowLeft as ArrowLeft, IconLoader2 as Loader2, IconExclamationCircle as AlertCircle } from '@tabler/icons-react';
...
const TeacherMessages: React.FC = () => {
  ...
  const fetchMessages = async (conv: Conversation) => {
    try {
      ...
      // Mark as read if there are unread messages from parent
      if (conv.unreadCount > 0) {
        // Logic to mark messages as read could be added here
      ...
      if (response.data.success) {
        ...
        setConversations(prev => prev.map(c => 
          (c.otherUser.id === selectedConv.otherUser.id && c.student.id === selectedConv.student.id)
            ? { ...c, lastMessage: { body: newMessage, createdAt: new Date().toISOString(), senderRole: 'TEACHER' } }
            : c
        ));
      }
    } catch (error: any) {
            ...
            filteredConversations.map((conv, idx) => (
              ...
              >
                ...
                <p className="text-sm text-slate-400 line-clamp-1">
                  {conv.lastMessage.senderRole === 'TEACHER' ? 'You: ' : ''}{conv.lastMessage.body}
                </p>
              </div>
            ))
          )}
      ...
      <div className={`flex-1 flex flex-col bg-black/20 ${!selectedConv ? 'hidden lg:flex' : 'flex'}`}>
        {selectedConv ? (
          <>
            ...
            <div className="p-4 border-b border-white/10 flex items-center gap-4 bg-white/5 backdrop-blur-md sticky top-0 z-10">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-400 truncate">Parent of {selectedConv.student.name}</p>
              </div>
            </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
            ...
            <p className="text-slate-400 max-w-sm">Select a conversation from the left to start messaging parents about their children's progress.</p>
          </div>
        )}
      </div>
    </div>
