...
export const Step1SchoolProfile = ({
  ...
  useEffect(() => {
    const saveChanges = async () => {
      ...
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_RESULTS_API || ''}/api/onboarding/school-profile`, {
          method: 'PATCH',
          headers: {
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_RESULTS_API || ''}/api/onboarding/logo-upload`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('authToken') || localStorage.getItem('accessToken')}`,
        },
