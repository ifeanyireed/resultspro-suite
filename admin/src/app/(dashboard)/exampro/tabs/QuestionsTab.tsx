...
import { BookOpen, Search, Filter, Plus, Upload, MoreVertical, RefreshCw } from 'lucide-react';
import { Badge } from '@/components/Badge';
import { fetchExamproQuestions } from '@/lib/api';
...
export default function QuestionsTab() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const data = await fetchExamproQuestions();
  ...
  const filteredQuestions = questions.filter(q => 
    q.bodyText?.toLowerCase().includes(search.toLowerCase()) || 
              ...
              {filteredQuestions.slice(0, 100).map((q) => (
                <tr key={q.id} className="hover:bg-purple-50/30 transition-colors group cursor-pointer">
                  <td className="px-6 py-4">
                    ...
                    <p className="font-bold text-slate-800 text-xs truncate max-w-sm">{q.bodyText || 'No text'}</p>
                  </td>
                  <td className="px-6 py-4">
