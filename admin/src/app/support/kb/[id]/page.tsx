...
export default function EditKBArticle() {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      const [catsRes, artRes] = await Promise.all([
        ...
        fetch(`${USERS_API}/api/v1/support/kb/articles/${articleId}`)
      ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const USERS_API = process.env.NEXT_PUBLIC_USERS_API || '';
      const res = await fetch(`${USERS_API}/api/v1/support/kb/articles/${articleId}`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50/50 p-6 rounded-2xl overflow-y-auto">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Edit Article</h2>
          <p className="text-sm text-gray-500 mt-1">Update knowledge base blueprint.</p>
        </div>
      </div>
