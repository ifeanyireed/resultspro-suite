...
BEGIN 
    FOR t_id IN (SELECT DISTINCT tenant_id FROM crs_programs UNION SELECT DISTINCT tenant_id FROM crs_cohorts) LOOP 
        IF t_id != 'skillupacademy' THEN 
            ...
            -- 1. Insert Cohort
            INSERT INTO crs_cohorts (id, tenant_id, program_id, slug, title, subtitle, description, status, meeting_days, meeting_time, location_type, difficulty_level, created_at, updated_at)
            VALUES (
