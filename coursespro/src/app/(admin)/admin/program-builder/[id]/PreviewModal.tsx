                        ...
                        {block.type === 'LIVE_CLASS' && (
                          <div className={`w-full ${isMediaWithoutCard ? 'py-4' : 'p-6 bg-white'}`}>
                            <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
                              <div className="flex items-start gap-4 flex-1">
                                ...
                                <div className="flex-1">
                                  <h3 className="text-lg font-bold text-emerald-900 tracking-tight">
                                    {block.class_type === 'PHYSICAL' ? 'Physical Class' : 'Live Class Room'}
                                  </h3>
                                  <p className="text-emerald-700 text-sm mt-1 mb-3">{block.description || 'Join your mentor and cohort peers for this live session.'}</p>
                                  
                                  <div className="flex flex-wrap gap-x-6 gap-y-2 mt-2">
                                    {(block.date || block.time) && (
                                      <div className="flex items-center gap-1.5 text-emerald-800 text-sm font-medium">
                                        ...
                                        {block.date ? new Date(block.date).toLocaleDateString() : 'Date TBD'}{block.time ? ` at ${block.time}` : ''}
                                      </div>
                                    )}

                                    {block.class_type === 'PHYSICAL' ? (
                                      <div className="flex flex-col gap-1 text-sm text-emerald-700">
                                        {block.address && (
                                        ...
                                        {block.classroom && (
                                          <div className="flex items-center gap-1.5 ml-5 text-emerald-600/80">
                                            <span className="text-xs font-semibold uppercase tracking-wider">Room:</span> {block.classroom}
                                          </div>
                                        )}
                                        {!block.address && !block.classroom && (
                                          <div className="text-xs text-emerald-600/80 italic">Location will be provided by your mentor.</div>
                                        )}
                                      </div>
                                    ) : (
                                      <div className="flex flex-col gap-1">
                                        {block.url ? (
                                          <div className="text-xs text-emerald-600/80 font-mono truncate max-w-[200px] sm:max-w-xs">{block.url}</div>
                                        ) : (
                              ...
                              <button 
                                className="w-full md:w-auto px-6 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-lg shadow-sm transition-colors flex items-center justify-center gap-2 whitespace-nowrap"
                                disabled={block.class_type === 'PHYSICAL' ? false : !block.url}
                              >
                                {block.class_type === 'PHYSICAL' ? (
                                  <>
