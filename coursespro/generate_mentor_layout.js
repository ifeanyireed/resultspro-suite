...
const promoStart = content.indexOf('{/* Bottom App Promo */}');

if (menuStart !== -1 && promoStart !== -1) {
  const newLinks = [
    '            {/* Menu Sections */}',
  ...
  content = content.substring(0, menuStart) + newLinks + content.substring(promoStart);
  
  // Fix isActive
  content = content.replace(
    /const isActive =.*/g,
    "const isActive = (path: string) => { if (path === '/mentor') return pathname === '/mentor'; return pathname === path || pathname.startsWith(path + '/'); };"
  );
  
  fs.writeFileSync(layoutPath, content);
  console.log('Mentor layout updated.');
} else {
