...
const srcLayout = path.join(__dirname, 'src', 'app', '(app)', 'layout.tsx');
const destLayout = path.join(__dirname, 'src', 'app', '(mentor)', 'layout.tsx');

let content = fs.readFileSync(srcLayout, 'utf8');

// Replace MENTOR links
const newLinks = \`
            {/* Menu Sections */}
...
const menuStart = content.indexOf('{/* Menu Sections */}');
const menuEnd = content.indexOf('{/* Bottom App Promo */}');

content = content.substring(0, menuStart) + newLinks + '\\n          ' + content.substring(menuEnd);

content = content.replace(/const isActive[\s\S]*?return \(/, 
