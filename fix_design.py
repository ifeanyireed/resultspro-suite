      ...
      <div className="container-nets relative z-10" style={{ marginTop: '-15vh' }}>
        <div className="max-w-[900px] mx-auto flex gap-8">
          ...
          <div className="hidden lg:flex flex-col items-center gap-4 mt-32 w-12 shrink-0">
            ...
            <button className="w-8 h-8 flex items-center justify-center rounded-full text-slate-400 hover:text-navy hover:bg-slate-200 transition-colors">
          ...
          <div className="bg-white shadow-xl flex-1">
            <ScrollReveal animation="fade-up">
              ...
              <div className="p-8 md:p-16 border-b border-slate-100">
                <span className="text-xs fw-700 tracking-widest uppercase text-slate-500 mb-6 block">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div className="text-xs text-slate-400 fw-600">
                    5 minutes Read
                  </div>
                </div>
              </div>
              ...
              <div className="p-8 md:p-16">
                ...
                <div className="mt-20">
                    ...
                    <div className="space-y-8">
                      {post.comments.map((comment) => (
                        <div key={comment.id} className="flex gap-4">
                          ...
                          <div className="flex-1 bg-slate-50 p-5 rounded-tr-xl rounded-br-xl rounded-bl-xl border border-slate-100">
                            <div className="flex items-center justify-between mb-2">
                              <span className="fw-600 text-navy text-sm">{comment.author}</span>
                              <button className="text-xs font-semibold text-slate-500 hover:text-navy flex items-center gap-1">
                                <IconMessageCircle size={14} /> Reply
                              </button>
                            </div>
                            <span className="text-[10px] text-slate-400 uppercase tracking-wider block mb-3">
                              {new Date(comment.created_at).toLocaleDateString()}
                            </span>
                            <p className="text-sm text-slate-600 leading-relaxed m-0">{comment.content}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="mt-16 bg-slate-50 border border-slate-100 rounded-xl p-8">
                    <h4 className="text-lg fw-600 text-navy mb-6">Leave your Comments</h4>
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <input type="text" placeholder="First Name" className="w-full px-4 py-3 rounded-lg border border-slate-200 text-sm focus:outline-none focus:border-blue-500" />
                      <input type="text" placeholder="Last Name" className="w-full px-4 py-3 rounded-lg border border-slate-200 text-sm focus:outline-none focus:border-blue-500" />
                    </div>
                    <input type="email" placeholder="Email Address" className="w-full px-4 py-3 rounded-lg border border-slate-200 text-sm focus:outline-none focus:border-blue-500 mb-4" />
                    <textarea placeholder="Your Comment" rows={4} className="w-full px-4 py-3 rounded-lg border border-slate-200 text-sm focus:outline-none focus:border-blue-500 mb-6"></textarea>
                    <button className="px-6 py-3 bg-emerald-400 hover:bg-emerald-500 text-white fw-600 text-sm rounded-lg transition-colors">
                      Submit your Comment
                    </button>
                  </div>
                </div>
