...
export default function BlogPostContent({ post }: { post: BlogPost }) {
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_USERS_API || 'https://resultspro-service-users.onrender.com'}/api/v1/cms/blog/comments`, {
        method: "POST",
      ...
      if (res.ok) {
        ...
        setCommentFirstName(''); setCommentLastName(''); setCommentEmail(''); setCommentContent(''); setReplyingTo(null);
      } else {
  ...
  const handleSubscribeSubmit = async () => {
    ...
    setSubLoading(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_USERS_API || 'https://resultspro-service-users.onrender.com'}/api/v1/cms/blog/subscribe`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
