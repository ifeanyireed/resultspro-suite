      ...
      >
        <div className="container-nets" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
          ...
          <Link href="/" aria-label="ExamsPRO" style={{ display: 'flex', alignItems: 'center', flexShrink: 0, transition: 'opacity 0.2s ease' }} onMouseEnter={e => e.currentTarget.style.opacity = '0.8'} onMouseLeave={e => e.currentTarget.style.opacity = '1'}>
            <img src="/logo.png" alt="My School Academy Logo" style={{ height: '56px', width: 'auto', objectFit: 'contain' }} />
            <span style={{ marginLeft: '12px', fontSize: '1.25rem', fontWeight: 900, color: 'white', letterSpacing: '-0.05em' }}>My School Academy</span>
          </Link>
            ...
            {mounted && !isAuthenticated ? (
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
                ...
                <Link href="/dashboard" style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', textDecoration: 'none' }}>
                  ...
                  <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 'bold', color: 'white', overflow: 'hidden' }}>
                    <img src={`/avatars/character${((user?.id?.charCodeAt(0) || 0) % 20) + 1}.jpg`} alt={user?.name || 'User'} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  </div>
                </Link>
              </div>
            ) : null}
