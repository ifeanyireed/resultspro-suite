...
const dashboards = [
  { 
    id: 'classrooms', 
    name: 'ClassroomPRO', 
    title: 'Classroom Command', 
    desc: 'Manage physical and virtual learning spaces.',
    apiRoute: '/admin/classrooms',
    metrics: ['Active Classes', 'Attendance Rate', 'Teachers Present', 'Room Utilization']
  },
  { 
    id: 'tutors', 
    name: 'TutorsPRO', 
    title: 'Tutor Hub', 
    desc: 'Manage private tutors and online sessions.',
    apiRoute: '/admin/tutors',
    metrics: ['Active Tutors', 'Total Sessions', 'Student Satisfaction', 'Revenue (MTD)']
  },
  { 
    id: 'cohorts', 
    name: 'CoursesPRO', 
    title: 'Cohorts & Courses', 
    desc: 'Manage curriculums, cohorts, and learning paths.',
    apiRoute: '/admin/cohorts',
    metrics: ['Active Cohorts', 'Course Completion', 'New Enrollments', 'Active Modules']
  },
  { 
    id: 'results', 
    name: 'ResultsPRO', 
    title: 'Results Center', 
    desc: 'Analyze academic performance and grade distributions.',
    apiRoute: '/admin/results',
    metrics: ['Average GPA', 'Top Performers', 'Pass Rate', 'Assessments Taken']
  }
];
          ...
          <div className="w-full bg-white rounded-[1.5rem] p-6 shadow-sm border border-gray-100 flex flex-col">
            ...
            <div className="flex-1 flex flex-col gap-8 pb-2">
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <p className="text-base font-normal text-gray-900">New Enrollments</p>
                  <p className="text-sm text-gray-500 mt-0.5">5 users onboarded</p>
                </div>
              </div>
            </div>
          </div>
...
`;
  ...
  fs.writeFileSync(`src/components/Dashboard/${d.name}Dashboard.tsx`, code);
  
  const pageDir = `src/app/(portal)${d.apiRoute}`;
  if (!fs.existsSync(pageDir)) {
    fs.mkdirSync(pageDir, { recursive: true });
  }

  const pageCode = `import ${d.name}Dashboard from '@/components/Dashboard/${d.name}Dashboard';
...
`;
  ...
  fs.writeFileSync(`${pageDir}/page.tsx`, pageCode);
});
console.log('Successfully generated all modular dashboards matching reference UI styling.');
