...
const revenueStats = [
  { label: "Total Revenue", value: "₦142.8M", trend: "+12.5%", positive: true, icon: <DollarSign className="w-5 h-5" />, color: "text-green", bg: "bg-green/10" },
  { label: "Monthly Recurring", value: "₦12.4M", trend: "+8.2%", positive: true, icon: <TrendingUp className="w-5 h-5" />, color: "text-blue", bg: "bg-blue/10" },
  { label: "Active Subscriptions", value: "112", trend: "+5", positive: true, icon: <School className="w-5 h-5" />, color: "text-purple-400", bg: "bg-purple-400/10" },
  { label: "Pending Payouts", value: "₦1.2M", trend: "-2.1%", positive: false, icon: <Wallet className="w-5 h-5" />, color: "text-amber", bg: "bg-amber/10" },
];
  ...
  return (
    <div className="flex-1 pb-12 animate-in fade-in duration-500">
      ...
      <main className="p-8 space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <h2 className="text-2xl font-bold text-white mb-2">Financial Performance</h2>
            <p className="text-sm text-muted-foreground">Detailed breakdown of platform revenue and subscription health.</p>
          </div>
          <div className="flex gap-3">
             ...
             <Button className="bg-white text-navy font-bold h-12 px-6 hover:bg-white/90 shadow-lg shadow-white/10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
           ...
           <div className="lg:col-span-2 bg-white/5 border border-white/10 rounded-[32px] p-8">
              <div className="flex justify-between items-center mb-10">
                 <div>
                    <h3 className="text-lg font-bold text-white font-display">Revenue Over Time</h3>
                    <p className="text-xs text-muted-foreground mt-1">Comparing 2024 vs 2025 performance</p>
                 </div>
                 <div className="flex gap-4">
                    <div className="flex items-center gap-2">
                       <div className="w-2 h-2 rounded-full bg-blue" />
                       <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">2025</span>
                    </div>
                    <div className="flex items-center gap-2">
                       <div className="w-2 h-2 rounded-full bg-white/10" />
                       <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">2024</span>
                    </div>
                 </div>
              </div>
                 ...
                 {[40, 55, 45, 70, 85, 65, 95, 80, 75, 100, 110, 120].map((height, i) => (
                    <div key={i} className="flex-1 space-y-3 group">
                       <div className="relative w-full h-full flex items-end">
                          ...
                          />
                          <div 
                            className="absolute bottom-0 w-full bg-white/5 rounded-t-lg" 
                            style={{ height: `${height * 0.6}%` }} 
                          />
                       </div>
                       <div className="text-[10px] text-center text-muted-foreground font-bold uppercase">
           ...
           <div className="bg-white/5 border border-white/10 rounded-[32px] p-8">
              <h3 className="text-lg font-bold text-white font-display mb-8">Plan Distribution</h3>
              <div className="space-y-8">
                 <div className="space-y-3">
                    <div className="flex justify-between items-center text-xs">
                       <span className="font-bold text-white/80">Premium (Annual)</span>
                 ...
                 <div className="space-y-3">
                    <div className="flex justify-between items-center text-xs">
                       <span className="font-bold text-white/80">Premium (Monthly)</span>
                 ...
                 <div className="space-y-3">
                    <div className="flex justify-between items-center text-xs">
                       <span className="font-bold text-white/80">Standard</span>
                 ...
                 <div className="space-y-3">
                    <div className="flex justify-between items-center text-xs">
                       <span className="font-bold text-white/80">Other</span>
              ...
              <div className="mt-12 p-6 rounded-2xl bg-white/[0.02] border border-white/5">
                 <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-xs font-bold text-white uppercase tracking-widest">Next Payout</span>
                 </div>
                 <p className="text-2xl font-bold text-white">₦1,240,500</p>
                 <p className="text-[10px] text-muted-foreground mt-1">Scheduled for Feb 1st, 2025</p>
              </div>
           </div>
        </div>
        ...
        {/* Transactions Table */}
        <div className="space-y-6">
           <h3 className="text-xl font-bold text-white font-display px-2">Recent Transactions</h3>
           <div className="bg-white/5 border border-white/10 rounded-[32px] overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm border-collapse">
                   <thead>
                      <tr className="border-b border-white/10 bg-white/5">
                         <th className="p-6 font-bold text-muted-foreground uppercase tracking-widest text-[10px]">Transaction ID</th>
                         ...
                         <th className="p-6 font-bold text-muted-foreground uppercase tracking-widest text-[10px]">Amount</th>
                         <th className="p-6 font-bold text-muted-foreground uppercase tracking-widest text-[10px]">Date</th>
                         <th className="p-6 font-bold text-muted-foreground uppercase tracking-widest text-[10px] text-right">Status</th>
                      </tr>
                   </thead>
                   <tbody className="divide-y divide-white/5">
                      {currentTransactions.map((tx) => (
                         <tr key={tx.id} className="hover:bg-white/[0.02] transition-colors group">
                            <td className="p-6 font-mono text-xs text-muted-foreground">{tx.id}</td>
                            <td className="p-6 font-bold text-white group-hover:text-green transition-colors">{tx.school}</td>
                            <td className="p-6 text-xs text-muted-foreground">{tx.plan}</td>
                            <td className="p-6 font-bold text-white">{tx.amount}</td>
                            <td className="p-6 text-xs text-muted-foreground">{tx.date}</td>
                            <td className="p-6 text-right">
                               <span className={cn("px-2 py-1 rounded text-[10px] font-bold uppercase", 
                                  tx.status === 'Successful' ? 'bg-green/10 text-green' : 'bg-amber/10 text-amber'
                               )}>{tx.status}</span>
                            </td>
                         </tr>
                      ))}
