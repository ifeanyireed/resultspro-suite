      ...
      <main className="p-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row gap-4">
           <div className="relative flex-1">
              ...
              <input 
                 ...
                 className="w-full pl-10 h-12 bg-white/5 border border-white/10 rounded-xl text-white outline-none focus:border-green transition-colors"
              />
           </div>
        ...
        <div className="grid gap-4">
           {currentItems.map((item) => (
              <div key={item.id} className="p-6 rounded-3xl bg-white/5 border border-white/10 hover:border-white/20 transition-all flex flex-col md:flex-row md:items-center gap-6 group">
                 <div className="w-14 h-14 rounded-2xl bg-navy border border-white/10 flex items-center justify-center text-muted-foreground group-hover:text-green transition-colors shrink-0">
                 ...
                 <div className="flex-1">
                    <div className="flex items-center gap-3 mb-1">
                       ...
                       <span className="px-2 py-0.5 rounded bg-white/5 border border-white/10 text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{item.type}</span>
                    </div>
                    <p className="text-xs text-muted-foreground">
                       by <span className="text-white font-bold">{item.author}</span> • {item.school} • Submitted {item.date}
                    </p>
                 ...
                 <div className="flex items-center gap-3">
                    <Button variant="outline" className="border-white/10 text-white hover:bg-white/5 h-11 px-6 font-bold text-xs">
                       <Eye className="w-4 h-4 mr-2" /> Review
                    </Button>
                    <div className="flex gap-2">
                       <button className="w-11 h-11 rounded-xl bg-green/10 border border-green/20 text-green flex items-center justify-center hover:bg-green hover:text-navy transition-all">
           ...
           {currentItems.length === 0 && (
              <div className="py-20 text-center space-y-4">
                 <div className="w-20 h-20 rounded-full bg-white/5 flex items-center justify-center mx-auto text-muted-foreground opacity-20">
        ...
        {/* Pagination Footer */}
        {totalItems > limit && (
           <div className="mt-8 bg-white/5 border border-white/10 rounded-[32px] p-6 flex flex-col md:flex-row items-center justify-between gap-4">
