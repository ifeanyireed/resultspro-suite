...
export default function SchoolLeaderboardPage() {
  ...
  return (
    <div className="flex-1 bg-[#146ef5] min-h-screen">
      
      
      <main className="p-8 max-w-5xl mx-auto">
        {/* Header Section from Frontend UI */}
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-50 border border-amber/20 text-amber-600 mb-6">
            <Trophy className="w-4 h-4" />
            <span className="text-xs font-bold uppercase tracking-widest">Hall of Fame</span>
          </div>
          <h1 className="text-4xl md:text-6xl font-display font-black text-gray-900 mb-6 uppercase">
            {activeTab} <span className="text-[#146ef5]">RANKINGS</span>
          </h1>
          
          {/* Tabs from Frontend UI */}
          <div className="flex items-center justify-center gap-2 p-1.5 rounded-2xl bg-white shadow-sm border border-gray-100 w-fit mx-auto">
            {['Academic', 'Engagement', 'Streaks'].map((tab) => (
              <button 
                ...
                className={`
                  ...
                  ${activeTab === tab ? 'bg-blue text-gray-900 shadow-lg shadow-blue/20' : 'text-gray-500 hover:text-gray-900'}
                `}
              >
                {tab}
              </button>
            ...
            <div className="flex flex-col items-center gap-4 flex-1 max-w-[140px]">
              <div className="relative">
                <div className="w-20 h-20 md:w-24 md:h-24 rounded-full border-4 border-blue overflow-hidden p-1 bg-[#146ef5] flex items-center justify-center">
                  <div className="w-full h-full rounded-full bg-blue/20 flex items-center justify-center text-[#146ef5] font-bold text-xl">
                    {podium[0].avatar}
                  </div>
                </div>
              ...
              <div className="text-center">
                <div className="text-sm font-bold text-gray-900 mb-1 truncate w-24">{podium[0].name}</div>
                <div className="text-xs font-black text-[#146ef5]">{podium[0].points.toLocaleString()} pts</div>
          ...
          {podium[1] && (
            <div className="flex flex-col items-center gap-4 flex-1 max-w-[160px] -translate-y-8">
              <div className="relative">
                ...
                <div className="w-28 h-28 md:w-32 md:h-32 rounded-full border-4 border-amber overflow-hidden p-1 bg-[#146ef5] flex items-center justify-center shadow-[0_0_30px_rgba(255,111,0,0.2)]">
                  <div className="w-full h-full rounded-full bg-amber/20 flex items-center justify-center text-amber-600 font-bold text-2xl">
                    {podium[1].avatar}
                  </div>
                </div>
              ...
              <div className="text-center">
                <div className="text-base font-black text-gray-900 mb-1 truncate w-32">{podium[1].name}</div>
                <div className="text-sm font-black text-amber-600">{podium[1].points.toLocaleString()} pts</div>
                <div className="text-[10px] text-amber-600/60 font-bold uppercase tracking-wider">{podium[1].class}</div>
              </div>
            ...
            <div className="flex flex-col items-center gap-4 flex-1 max-w-[140px]">
              <div className="relative">
                <div className="w-20 h-20 md:w-24 md:h-24 rounded-full border-4 border-green overflow-hidden p-1 bg-[#146ef5] flex items-center justify-center">
                  <div className="w-full h-full rounded-full bg-emerald-100 flex items-center justify-center text-emerald-600 font-bold text-xl">
                    {podium[2].avatar}
                  </div>
                </div>
              ...
              <div className="text-center">
                <div className="text-sm font-bold text-gray-900 mb-1 truncate w-24">{podium[2].name}</div>
                <div className="text-xs font-black text-emerald-600">{podium[2].points.toLocaleString()} pts</div>
        ...
        <div className="space-y-3 max-w-4xl mx-auto">
          {others.map((student, index) => (
            <div 
              key={student.id}
              className="group p-4 rounded-2xl bg-white/2 border border-white/5 hover:bg-gray-50 hover:border-gray-100 transition-all flex items-center justify-between"
            >
              <div className="flex items-center gap-4">
                <div className="w-8 text-sm font-black text-gray-600">{index + 4}</div>
                <div className="w-10 h-10 rounded-full border border-gray-100 bg-white shadow-sm border border-gray-100 flex items-center justify-center text-xs font-bold text-gray-900">
                  {student.avatar}
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <TrendingUp className={`w-3 h-3 ${student.trend === 'up' ? 'text-emerald-600' : student.trend === 'down' ? 'text-red-500' : 'text-gray-500'}`} />
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-8">
                <div className="text-right">
                  <div className="text-sm font-black text-gray-900">{student.points.toLocaleString()}</div>
        ...
        <div className="mt-12 flex justify-center">
           <Button variant="outline" className="border-gray-100 hover:bg-gray-50 text-gray-400 hover:text-gray-900 rounded-xl px-8">
