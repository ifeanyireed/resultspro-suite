...
const schoolStatsData = [
  { label: "Active Students", value: "1,240", trend: "+12%", positive: true, icon: <Users className="w-5 h-5" />, color: "text-[#146ef5]", bg: "bg-blue-50" },
  { label: "Content Created", value: "458", trend: "+24", positive: true, icon: <BookOpen className="w-5 h-5" />, color: "text-emerald-600", bg: "bg-emerald-50" },
  { label: "Avg. Test Score", value: "76%", trend: "+3.4%", positive: true, icon: <BrainCircuit className="w-5 h-5" />, color: "text-amber-600", bg: "bg-amber-50" },
  { label: "Engagement Rate", value: "88%", trend: "-2%", positive: false, icon: <TrendingUp className="w-5 h-5" />, color: "text-purple-600", bg: "bg-purple-50" },
];
  ...
  return (
    <div className="flex-1">
      ...
      <main className="p-8 space-y-8">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 font-display">Academic Analytics</h2>
            <p className="text-sm text-gray-500">Comprehensive insights into your school's performance.</p>
          </div>
          <div className="flex gap-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {schoolStatsData.map((stat, i) => (
            <div key={i} className="bg-white shadow-sm border border-gray-100 p-6 rounded-[24px]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
           ...
           <div className="lg:col-span-2 bg-white shadow-sm border border-gray-100 rounded-[32px] p-8">
              <h3 className="text-lg font-bold text-gray-900 font-display mb-8">Performance by Class</h3>
              <div className="space-y-6">
                 {[
                    { class: "SSS 3", score: 85, color: "bg-emerald-600" },
                    ...
                    { class: "JSS 1", score: 81, color: "bg-emerald-600" },
                 ].map((item, i) => (
                    <div key={i} className="space-y-2">
                       <div className="flex justify-between text-xs font-bold text-gray-900/80">
           ...
           <div className="bg-white shadow-sm border border-gray-100 rounded-[32px] p-8">
              <h3 className="text-lg font-bold text-gray-900 font-display mb-8">Engagement by Subject</h3>
              <div className="aspect-square relative flex items-center justify-center mb-8">
                 ...
                 <div className="w-48 h-48 rounded-full border-[16px] border-green/20 border-t-green border-r-blue/80 border-l-amber/60 flex items-center justify-center">
                    <div className="text-center">
                       ...
                       <div className="text-[8px] text-gray-500 uppercase font-bold tracking-widest">Global Avg.</div>
                    </div>
                 </div>
              </div>
              <div className="space-y-3">
                 {[
                    { label: "Science", val: "42%", color: "bg-emerald-600" },
                    ...
                    <div key={i} className="flex items-center justify-between text-[10px] font-bold uppercase tracking-widest">
                       ...
                       <span className="text-gray-900">{it.val}</span>
                    </div>
                 ))}
              </div>
           </div>
