    ...
    <div className="flex-1 pb-12">
      ...
      <main className="p-8 max-w-7xl mx-auto space-y-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h2 className="text-3xl font-bold text-gray-900 mb-2 font-display">Academic Assessments</h2>
            <p className="text-gray-500 text-sm">Track your scheduled exams and past performance.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
           <div className="bg-white shadow-sm border border-gray-100 p-6 rounded-[32px] flex items-center gap-4 relative overflow-hidden group">
              ...
              <div>
                 ...
                 <div className="text-[10px] text-gray-500 uppercase font-bold tracking-widest">Total Exams</div>
              </div>
           </div>
           <div className="bg-white shadow-sm border border-gray-100 p-6 rounded-[32px] flex items-center gap-4 relative overflow-hidden group">
        ...
        <div className="flex flex-col md:flex-row gap-4 bg-white shadow-sm border border-gray-100 p-4 rounded-2xl border border-gray-100 backdrop-blur-xl">
          ...
          <div className="flex flex-wrap gap-2">
             <Select value={selectedStatus} onValueChange={setSelectedStatus}>
                <SelectTrigger className="w-[160px] bg-[#146ef5] border-gray-100 text-white h-12 rounded-xl">
        ...
        <div className="space-y-12">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {currentExams.map((exam) => (
              <div key={exam.id} className="bg-white shadow-sm border border-gray-100 rounded-[32px] p-8 hover:border-green/50 transition-all group flex flex-col md:flex-row gap-8 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-600/5 blur-[40px] rounded-full -z-10" />
                
                <div className="flex flex-col items-center md:items-start text-center md:text-left gap-4">
                  <div className="w-16 h-16 rounded-2xl bg-blue-50 flex items-center justify-center text-[#146ef5] group-hover:scale-110 transition-transform shadow-xl">
                    <GraduationCap className="w-8 h-8" />
                  </div>
                  <div>
                    <h3 className="text-xl font-bold text-gray-900 mb-1 group-hover:text-emerald-600 transition-colors">{exam.title}</h3>
                    <p className="text-xs text-gray-500 uppercase tracking-widest font-bold">{exam.class?.name || "General"}</p>
                  </div>
                </div>
                ...
                <div className="flex-1 space-y-6">
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    <div className="space-y-1">
                      ...
                      <p className="text-sm font-bold text-gray-900 flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-[#146ef5]" /> {exam.duration}m</p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Questions</p>
                      <p className="text-sm font-bold text-gray-900 flex items-center gap-1.5"><HelpCircle className="w-3.5 h-3.5 text-[#146ef5]" /> {exam.totalQuestions}</p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Best Score</p>
                      <p className="text-sm font-bold text-emerald-600 flex items-center gap-1.5"><Trophy className="w-3.5 h-3.5" /> {exam.bestScore || 0}%</p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col md:flex-row gap-3 pt-6 border-t border-white/5">
                    {exam.status === "UPCOMING" ? (
                      <Link href={`/dashboard/exams/${exam.id}`} className="flex-1">
                        <Button className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold h-11 rounded-xl shadow-lg shadow-green/10">
                          Start Exam
                        </Button>
                      </Link>
                    ) : (
                      <Button disabled className="flex-1 bg-gray-100 text-gray-500 font-bold h-11 rounded-xl border border-gray-100 cursor-not-allowed">
                        Already Taken
                      </Button>
                    )}
                    <Button variant="outline" className="border-gray-100 hover:bg-gray-100 text-gray-900 h-11 px-6 rounded-xl font-bold text-xs">
                      View Guidelines
                    </Button>
                  </div>
                </div>
