    ...
    <div className="flex-1 pb-12">
      ...
      <main className="p-8 space-y-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <h2 className="text-3xl font-bold text-white mb-2 font-display">Hall of Fame 🏆</h2>
            <p className="text-muted-foreground text-sm">Compete with your peers and climb the ranks.</p>
          </div>
        ...
        <div className="max-w-2xl mx-auto space-y-12">
                ...
                <div className="flex flex-col items-center gap-4 flex-1 max-w-[140px]">
                  <div className="relative">
                    <div className="w-20 h-20 md:w-24 md:h-24 rounded-full border-4 border-blue overflow-hidden p-1 bg-navy flex items-center justify-center">
                       <div className="w-full h-full rounded-full bg-blue/20 flex items-center justify-center font-black text-2xl text-white overflow-hidden">
                  ...
                  <div className="text-center">
                    <div className="text-sm font-bold text-white mb-1 truncate w-24">{podium[0].user?.full_name || 'Anonymous'}</div>
                    <div className="text-xs font-black text-blue">{podium[0].xp?.toLocaleString()}</div>
                  </div>
              ...
              {podium[1] && (
                <div className="flex flex-col items-center gap-4 flex-1 max-w-[160px] -translate-y-8">
                  <div className="relative">
                    ...
                    <div className="w-28 h-28 md:w-32 md:h-32 rounded-full border-4 border-amber overflow-hidden p-1 bg-navy shadow-[0_0_30px_rgba(255,111,0,0.2)] flex items-center justify-center">
                       <div className="w-full h-full rounded-full bg-amber/20 flex items-center justify-center font-black text-4xl text-white overflow-hidden">
                  ...
                  <div className="text-center">
                    <div className="text-base font-black text-white mb-1 truncate w-32">{podium[1].user?.full_name || 'Anonymous'}</div>
                    <div className="text-sm font-black text-amber">{podium[1].xp?.toLocaleString()}</div>
                  </div>
                ...
                <div className="flex flex-col items-center gap-4 flex-1 max-w-[140px]">
                  <div className="relative">
                    <div className="w-20 h-20 md:w-24 md:h-24 rounded-full border-4 border-green overflow-hidden p-1 bg-navy flex items-center justify-center">
                       <div className="w-full h-full rounded-full bg-green/20 flex items-center justify-center font-black text-2xl text-white overflow-hidden">
                  ...
                  <div className="text-center">
                    <div className="text-sm font-bold text-white mb-1 truncate w-24">{podium[2].user?.full_name || 'Anonymous'}</div>
                    <div className="text-xs font-black text-green">{podium[2].xp?.toLocaleString()}</div>
                  </div>
                ...
                return (
                  <div 
                    ...
                    className={cn(
                      ...
                      isMe 
                        ? "bg-blue/10 border-blue/20 hover:bg-blue/20" 
                        : "bg-white/[0.02] border-white/[0.05] border-t-white/[0.1] hover:bg-white/5 hover:border-white/10"
                    )}
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-8 text-sm font-black text-gray-600">{index + 4}</div>
                      <div className="w-10 h-10 rounded-full border border-white/10 overflow-hidden bg-navy flex items-center justify-center font-bold text-white">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                           <div className="text-sm font-bold text-white">{profile.user?.full_name || 'Anonymous'}</div>
                           {isMe && <span className="px-2 py-0.5 rounded bg-blue text-white text-[8px] font-black uppercase tracking-tighter">You</span>}
                           {profile.user?.role === 'TEACHER' && <span className="px-2 py-0.5 rounded bg-amber/10 text-amber border border-amber/20 text-[8px] font-black uppercase tracking-tighter">Staff</span>}
                        </div>
                    ...
                    <div className="flex items-center gap-8">
                      <div className="text-right">
                        <div className="text-sm font-black text-white">{profile.xp?.toLocaleString()}</div>
            ...
            {user && myProfile && (
              <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-full max-w-2xl px-4 z-30">
                <div className="p-4 rounded-2xl bg-blue border border-white/20 shadow-[0_0_40px_rgba(21,101,192,0.5)] flex items-center justify-between animate-in slide-in-from-bottom-8 duration-500 backdrop-blur-xl">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-navy/50 flex items-center justify-center font-black text-white text-lg border border-white/10 shadow-inner">
                      {myRank}
                    </div>
                    <div className="w-10 h-10 rounded-full border-2 border-white/20 overflow-hidden bg-navy flex items-center justify-center font-bold text-white">
                      {user.avatar_url ? (
                    ...
                    <div>
                      <div className="text-sm font-black text-white">You ({user.full_name})</div>
                      <div className="text-[10px] text-white/70 font-bold uppercase tracking-widest mt-0.5">
                  ...
                  <div className="flex items-center gap-8">
                    <div className="text-right">
                      <div className="text-sm font-black text-white">{myProfile.xp?.toLocaleString()}</div>
                      <div className="text-[10px] text-white/70 font-bold uppercase tracking-widest">XP</div>
                    </div>
                  </div>
                </div>
              </div>
