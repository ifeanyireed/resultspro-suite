  ...
  if (loading) {
    return (
      <div className="flex-1 pb-12 animate-in fade-in duration-500">
        <DashboardHeader title="My Flashcards" />
    ...
    <div className="flex-1 pb-12">
      ...
      <main className="p-8 space-y-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h2 className="text-3xl font-bold text-white mb-2 font-display">Study Sets</h2>
            <p className="text-muted-foreground text-sm">Use active recall to master difficult topics faster.</p>
          </div>
          <div className="flex gap-3">
             <Link href="/dashboard/flashcards/analytics">
                <Button variant="outline" className="border-white/10 text-white font-bold h-11 px-6 rounded-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
           <div className="bg-white/5 border border-white/10 p-6 rounded-[32px] flex items-center gap-4 relative overflow-hidden group">
              ...
              <div>
                 ...
                 <div className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">Total Cards</div>
              </div>
           </div>
           <div className="bg-white/5 border border-white/10 p-6 rounded-[32px] flex items-center gap-4 relative overflow-hidden group">
           ...
           <div className="bg-white/5 border border-white/10 p-6 rounded-[32px] flex items-center gap-4 relative overflow-hidden group">
              ...
              <div>
                 ...
                 <div className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">Sets Mastered</div>
              </div>
           </div>
           <div className="bg-white/5 border border-white/10 p-6 rounded-[32px] flex items-center gap-4 relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-24 h-24 bg-blue/5 blur-2xl rounded-full" />
              ...
              <div>
                 ...
                 <div className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">Longest Streak</div>
              </div>
           </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 bg-white/5 p-4 rounded-2xl border border-white/10 backdrop-blur-xl">
          ...
          <div className="flex flex-wrap gap-2">
             <Select value={selectedTerm} onValueChange={(val) => { setSelectedTerm(val); setCurrentPage(1); }}>
                <SelectTrigger className="w-[130px] bg-navy border-white/10 text-white h-12 rounded-xl">
             ...
             <Select value={selectedSubjectId || "all"} onValueChange={(val) => { setSelectedSubjectId(val === "all" ? null : val); setCurrentPage(1); }}>
                <SelectTrigger className="w-[150px] bg-navy border-white/10 text-white h-12 rounded-xl">
             ...
             <Select value={selectedStatus} onValueChange={(val) => { setSelectedStatus(val); setCurrentPage(1); }}>
                <SelectTrigger className="w-[140px] bg-navy border-white/10 text-white h-12 rounded-xl">
        ...
        <div className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {currentSets.map((set) => (
              <div key={set.id} className={cn(
                "bg-white/5 border border-white/10 rounded-[32px] p-6 transition-all group relative overflow-hidden flex flex-col",
                ...
                <div className="flex justify-between items-start mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{set.items?.length || 0} Cards</span>
                      <div className="flex items-center gap-2 mt-1">
                        <span className={cn(
                          "text-[9px] font-bold px-1.5 py-0.5 rounded border border-white/10 w-fit uppercase tracking-wider",
                          set.author?.role === 'TEACHER' ? "bg-green/10 text-green border-green/20" : "bg-blue/10 text-blue border-blue/20"
                        )}>
                          {set.author?.name || 'Jane Doe'}
                        </span>
                        {set.dueCount === 0 && set.lastStudied && (
                          <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-amber/10 text-amber border border-amber/20 uppercase tracking-wider">
                            Waiting
                          </span>
                        )}
                      </div>
                ...
                <h3 className="text-lg font-bold text-white mb-1 line-clamp-1 group-hover:text-blue transition-colors">{set.title}</h3>
                <div className="flex flex-col gap-1 mb-6">
                  <p className="text-xs text-muted-foreground">
                    {set.note?.topic?.subject?.name || "General"} • {set.note?.topic?.term === 1 ? '1st' : set.note?.topic?.term === 2 ? '2nd' : '3rd'} Term
                  </p>
                  {set.lastStudied && (
                    <p className="text-[10px] text-green/60 font-medium flex items-center gap-1">
                ...
                <div className="mt-auto space-y-4">
                  <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest">
                      ...
                      <span className="text-blue">{set.mastery || 0}%</span>
                  </div>
                  ...
                  <div className="flex gap-2 pt-2">
                      {(set.dueCount > 0 || !set.lastStudied) ? (
                        <Link href={`/dashboard/flashcards/${set.id}`} className="flex-1">
                          <Button className="w-full bg-blue hover:bg-blue/80 text-white font-bold text-xs h-10 rounded-xl shadow-lg shadow-blue/20">
                            Study Now
                          </Button>
                        </Link>
                      ) : (
                        <>
                          <Button disabled className="flex-1 bg-white/10 text-muted-foreground font-bold text-xs h-10 rounded-xl border border-white/10 cursor-not-allowed">
                            Practiced
                          </Button>
                          <Link href={`/dashboard/flashcards/${set.id}?mode=all`}>
                            <Button variant="outline" className="border-white/10 hover:bg-white/10 text-white h-10 w-10 p-0 rounded-xl" title="Bypass SRS">
          ...
          {totalItems > limit && (
            <div className="mt-12 flex flex-col md:flex-row items-center justify-between gap-8 bg-white/5 p-6 rounded-[32px] border border-white/10 backdrop-blur-xl">
               <div className="flex items-center gap-4">
                  <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Items per page</span>
                  <Select value={itemsPerPage} onValueChange={setItemsPerPage}>
                    <SelectTrigger className="w-20 bg-navy/50 border-white/10 text-white h-10 rounded-xl focus:ring-0 ring-0">
          ...
          {filteredSets.length === 0 && (
            <div className="col-span-full text-center py-20 bg-white/5 rounded-[40px] border border-white/10 border-dashed">
               ...
               <p className="text-muted-foreground">No study sets found matching your criteria.</p>
               <button 
