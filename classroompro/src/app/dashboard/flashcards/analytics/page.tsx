  ...
  if (loading) {
    return (
      <div className="flex-1 pb-12 animate-in fade-in duration-500">
        <DashboardHeader title="Flashcard Analytics" />
      ...
      <main className="p-8 max-w-7xl mx-auto space-y-8">
        <div className="flex items-center justify-between">
           <div className="flex items-center gap-4">
              ...
              <div>
                 <h2 className="text-3xl font-bold text-white font-display">Command Center</h2>
                 <p className="text-muted-foreground text-sm">Visualize your memory retention and study habits.</p>
              </div>
           </div>
           ...
           <div className="flex items-center gap-3 bg-white/5 border border-white/10 p-2 rounded-2xl backdrop-blur-md">
              <div className="px-4 py-2 flex flex-col items-center">
                 ...
                 <span className="text-[8px] font-bold text-muted-foreground uppercase tracking-widest">Streak</span>
              </div>
              ...
              <div className="px-4 py-2 flex flex-col items-center">
                 ...
                 <span className="text-[8px] font-bold text-muted-foreground uppercase tracking-widest">Total XP</span>
              </div>
              <div className="w-px h-8 bg-white/10" />
              <div className="px-4 py-2 flex flex-col items-center">
                 ...
                 <span className="text-[8px] font-bold text-muted-foreground uppercase tracking-widest">Level</span>
              </div>
           </div>
        </div>
        ...
        <div className="bg-white/5 border border-white/10 p-8 rounded-[40px] relative overflow-hidden">
           ...
           <div className="relative z-10 space-y-4">
              <div className="flex justify-between items-end">
                 <div>
                    <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-[0.2em] mb-1">Current Level</h3>
                    <p className="text-3xl font-black text-white">Level {profile?.profile?.level || 1}</p>
                 </div>
                 <div className="text-right">
                    <p className="text-xs font-bold text-white mb-1">{(profile?.profile?.xp % 500) || 0} / 500 XP</p>
                    <p className="text-[10px] text-muted-foreground uppercase tracking-widest">Next Level: {((profile?.profile?.level || 1) + 1)}</p>
                 </div>
              </div>
              <div className="h-4 w-full bg-navy rounded-full overflow-hidden p-1 border border-white/5">
                 <div 
        ...
        <div className="grid lg:grid-cols-3 gap-8">
           {/* Retention Forecast */}
           <div className="lg:col-span-1 bg-white/5 border border-white/10 rounded-[32px] p-6 flex flex-col">
              <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
                 <Layers className="w-5 h-5 text-blue" /> Retention Forecast
              </h3>
              ...
              <div className="grid grid-cols-2 gap-4 mt-6">
                 {retentionData.map((item: any) => (
                    <div key={item.name} className="flex items-center gap-2">
                       ...
                       <span className="text-xs text-muted-foreground font-bold uppercase">{item.name}</span>
           ...
           {/* Performance Trends */}
           <div className="lg:col-span-2 bg-white/5 border border-white/10 rounded-[32px] p-6">
              <div className="flex items-center justify-between mb-8">
                 <h3 className="text-lg font-bold text-white flex items-center gap-2">
                    <TrendingUp className="w-5 h-5 text-green" /> Success Rates
                 </h3>
                 <div className="flex gap-2">
                    <span className="px-3 py-1 rounded-full bg-green/10 text-green text-[10px] font-bold">AVG: 84%</span>
                 </div>
              </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
           ...
           <div className="bg-white/5 border border-white/10 rounded-[32px] p-6">
              <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
                 <Clock className="w-5 h-5 text-amber-500" /> Study Time (Minutes)
              </h3>
           ...
           <div className="bg-white/5 border border-white/10 rounded-[32px] p-6">
              <div className="flex items-center justify-between mb-6">
                 ...
                 <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Identified Leeches</span>
              </div>
                 ...
                 {leeches.map((leech, i) => (
                    <div key={i} className="p-4 rounded-2xl bg-red-500/5 border border-red-500/10 flex items-center justify-between group hover:bg-red-500/10 transition-all">
                       <div className="flex items-center gap-4">
                          ...
                          <div>
                             <h4 className="font-bold text-white text-sm group-hover:text-red-400 transition-colors">{leech.title}</h4>
                             <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-tighter">{leech.subject}</p>
                          </div>
                       </div>
                       <div className="text-right">
                          <p className="text-xs font-black text-red-500">{leech.count} fails</p>
                          <button className="text-[9px] font-bold text-muted-foreground hover:text-white underline">Target Study</button>
                       </div>
                    </div>
                 ))}
              </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-[32px] p-8">
           <h3 className="text-xl font-bold text-white mb-8 flex items-center gap-2">
              <Award className="w-6 h-6 text-green" /> Achievements
           </h3>
                ...
                { name: "Unstoppable", icon: "🛡️", desc: "100 correct in a row" },
                 ...
                 return (
                    <div key={badge.name} className={cn(
                      ...
                      isEarned ? "bg-white/5 border-green/20" : "bg-white/[0.02] border-white/5 grayscale opacity-40"
                    )}>
                       <div className="text-4xl">{badge.icon}</div>
                       <div>
                          <p className="text-xs font-bold text-white mb-1">{badge.name}</p>
                          <p className="text-[8px] text-muted-foreground leading-tight">{badge.desc}</p>
                       </div>
                    </div>
                 );
              })}
