...
with open('landing_page/src/app/(marketing)/blog/page.tsx', 'r') as f:
    content = f.read()

# Replace the array mapping
new_render = """      <section className="section-py">
...
content = re.sub(r'<section className="section-py">\s*<div className="container-nets">\s*<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">.*?</div>', new_render, content, flags=re.DOTALL)

# Add async data fetching
fetch_code = """async function getBlogPosts() {
  try {
...
content = content.replace('export default function BlogPage() {', fetch_code)

# Remove DUMMY_POSTS
content = re.sub(r'const DUMMY_POSTS = \[.*?\];\n\n', '', content, flags=re.DOTALL)

with open('landing_page/src/app/(marketing)/blog/page.tsx', 'w') as f:
    f.write(content)
